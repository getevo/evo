...
type runner struct {
	bin          string
	args         []string
	writerStdout evo.Writer
	writerStderr evo.Writer
	command      *exec.Cmd
	starttime    time.Time
	errors       chan error
...
func NewRunner(wStdout evo.Writer, wStderr evo.Writer, bin string, args []string) Runner {
	return &runner{
		bin:          bin,
		args:         args,
...
func (r *runner) runBin() error {
	...
	go evo.Copy(r.writerStdout, stdout) // nolint errcheck
	go evo.Copy(r.writerStderr, stderr) // nolint errcheck
