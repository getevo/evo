...
EVO.defaultCookieAttributes = { expires: 365, path:"", }
...
/**
 * Initialize the EVO preparations on each load
 */
EVO.Init = function(){
    $.ajaxSetup({
...
EVO.defaultAjaxAttributes = {dataType:"JSON", data:[], type:"POST", error:function (error) { if(EVO.onAjaxError != undefined) EVO.onAjaxError(error);}}
EVO.Ajax = function(params){
    params = EVO.Assign({},EVO.defaultAjaxAttributes,params)
    $.ajax(params)
}
 ...
 */
EVO.Cookie = function (key,set,attributes) {
  ...
  if(set == undefined){
      ...
      attributes = EVO.Assign({}, EVO.defaultCookieAttributes, attributes)
 ...
 */
EVO.RemoveCookie = function(key){
    if (typeof document === 'undefined' || (arguments.length && !key)) {
        return
    }
    EVO.Cookie(key,"",{expires:-100000000 , path: "/"})
}
 ...
 */
EVO.Alert = function(message,onclose){
 ...
 */
EVO.Confirm = function(message,yes,no){

}

EVO.Loading = function(timeout){
 ...
 */
EVO.Message = function(message,type){
 ...
 * @param type is message type (default:info)
 */
EVO.Redirect = function(url,message,type){
    if(message != undefined){
        EVO.Message(message,type)
    }
    window.location = EVO.Route(url)
}
 ...
 * @param type is message type (default:info)
 */
EVO.Back = function(message,type){
    if(message != undefined){
        EVO.Message(message,type)
    }
    window.history.back()
 ...
 * @param type is message type (default:info)
 */
EVO.Refresh = function(message,type){
    if(message != undefined){
        EVO.Message(message,type)
    }
 ...
 */
EVO.Route = function(path){
    r = new RegExp('^(?:[a-z]+:)?//', 'i');
    if(!r.test(path)){
        path = EVO.Base().trim("/")+"/"+path
    }
    return path;
}
