...
func TestContextPool(t *testing.T) {
	...
	t.Run("panics on configuration after init", func(t *testing.T) {
		t.Run("before wait", func(t *testing.T) {
			t.Parallel()
			g := NewPool().WithContext(context.Background())
			g.Exec(func(context.Context) error { return nil })
	...
	t.Run("behaves the same as ErrorGroup", func(t *testing.T) {
		...
		t.Run("wait returns no error if no errors", func(t *testing.T) {
			...
			p.Exec(func(context.Context) error { return nil })
		...
		t.Run("wait errors if func returns error", func(t *testing.T) {
			t.Parallel()
			p := NewPool().WithContext(bgctx)
			p.Exec(func(context.Context) error { return err1 })
			require.ErrorIs(t, p.Wait(), err1)
		})

		t.Run("wait error is all returned errors", func(t *testing.T) {
			t.Parallel()
			p := NewPool().WithErrors().WithContext(bgctx)
			p.Exec(func(context.Context) error { return err1 })
			p.Exec(func(context.Context) error { return nil })
			p.Exec(func(context.Context) error { return err2 })
			err := p.Wait()
	...
	t.Run("context error propagates", func(t *testing.T) {
		...
		t.Run("return before timed out", func(t *testing.T) {
			t.Parallel()
			p := NewPool().WithContext(context.Background())
			p.Exec(func(ctx context.Context) error {
				select {
				case <-ctx.Done():
					return ctx.Err()
	...
	t.Run("WithFailFast", func(t *testing.T) {
		t.Parallel()
		p := NewPool().WithContext(bgctx).WithFailFast()
		p.Exec(func(ctx context.Context) error {
			return err1
		})
		p.Exec(func(ctx context.Context) error {
	...
	t.Run("limit", func(t *testing.T) {
		...
		for _, maxConcurrent := range []int{1, 10, 100} {
			t.Run(strconv.Itoa(maxConcurrent), func(t *testing.T) {
				...
				t.Parallel()
				p := NewPool().WithContext(bgctx).WithMaxGoroutines(maxConcurrent)

				var currentConcurrent atomic.Int64
				for i := 0; i < 100; i++ {
					p.Exec(func(context.Context) error {
